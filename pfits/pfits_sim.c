...
// gcc -lm -o pfits_sim pfits_sim.c pfits_setup.c pfits_loader.c T2toolkit.c -I/Users/hob044/hob044/software/cfitsio/include/ -L/Users/hob044/hob044/software/cfitsio/lib -lcfitsio -lcpgplot -lpgplot
...
int main(int argc,char *argv[])
{
  ...
  int nBursts=0;
  int num_nb_rfi = 0;
  int nBurstsSub[MAX_BURSTS];
  burstStruct **burstsSub;
  int sub0,sub1,sub;
  float toff1,toff2,tt;
  float fref;
  ...
  // Sort out the subintegrations in which the bursts will occur
  burstsSub = (burstStruct **)malloc(sizeof(burstStruct *)*simData.nsub);
  for (i=0;i<simData.nsub;i++)
    burstsSub[i] = (burstStruct *)malloc(sizeof(burstStruct)*MAX_BURSTS);

  printf("Determining burst times\n");
  fref = simData.fchan0+simData.chanbw*simData.nchan/2.0;
  for (i=0;i<nBursts;i++)
    {
      toff1 = bursts[i].time-(4.15e-3*bursts[i].dm*(pow(fref/1000.0,bursts[i].si)-pow((simData.fchan0+simData.chanbw*simData.nchan)/1000.0,bursts[i].si)));
      toff2 = bursts[i].time-(4.15e-3*bursts[i].dm*(pow(fref/1000.0,bursts[i].si)-pow((simData.fchan0)/1000.0,bursts[i].si)));
      if (toff1 > toff2)
	{
	  tt = toff1;
	  toff1 = toff2;
      ...
      sub0 = (int)((toff1-bursts[i].width/2.0)/(simData.tsamp*simData.nsblk));
      sub1 = (int)((toff1+bursts[i].width/2.0)/(simData.tsamp*simData.nsblk)+1);
      for (sub=sub0;sub<=sub1;sub++)
	{
	  burstsSub[sub][nBurstsSub[sub]].time = bursts[i].time;
	  burstsSub[sub][nBurstsSub[sub]].dm = bursts[i].dm;
	  burstsSub[sub][nBurstsSub[sub]].flux = bursts[i].flux;
	  burstsSub[sub][nBurstsSub[sub]].width = bursts[i].width;
	  
	  nBurstsSub[sub]++;
	}
    }
  //  toff = 4.15e-3*bursts[sub][b].dm*(pow(fref/1000.0,-2)-pow((simData->fchan0+simData->chanbw*j)/1000.0,-2));
  //  if (bursts[sub][b].time-bursts[sub][b].width/2.0 <= tval+toff && bursts[sub][b].time+bursts[sub][b].width/2.0 > tval+toff)

  printf("Complete determining burst times\n");
    


  createSimulation(&simData,burstsSub,nBurstsSub,nb_rfi,num_nb_rfi);
...
void loadInputFile(char *fname,simDataStruct *simData,burstStruct *bursts,int *nBursts,nb_rfi_struct *nb_rfi,int *num_nb_rfi)
{
  ...
  while (!feof(fin))
      ...
      if (fgets(line,2048,fin)!=NULL)
	{
	  ...
	  if (strcmp(first,"name:")==0)
	  ...
	  else if (strcmp(first,"ra:")==0)
	    strcpy(simData->ra,second);
	  else if (strcmp(first,"dec:")==0)
	    strcpy(simData->dec,second);
	      ...
	      while( tok != NULL)
		{
		  ...
		  if (strcmp(tok2,"dm")==0)
 		  ...
 		  else if (strcmp(tok2,"si")==0)
		    sscanf(tok3,"%f",&(bursts[*nBursts].si));
		  else
		    {printf("ERROR: Unknown command: %s\n",tok2); exit(1);}
		  printf("Got here\n");
		}
  ...
  for (sub=0;sub<simData->nsub;sub++)
      ...
      for (i=0;i<simData->nsblk;i++)
	{
	  ...
	  for (p=0;p<simData->npol;p++)
		...
		{
		  val = TKgaussDev(&seed);
		  for (b=0;b<nBursts[sub];b++)
		    {
		      toff = 4.15e-3*bursts[sub][b].dm*(pow(fref/1000.0,bursts[sub][b].si)-pow((simData->fchan0+simData->chanbw*j)/1000.0,bursts[sub][b].si));
		      if (bursts[sub][b].time-bursts[sub][b].width/2.0 <= tval+toff && bursts[sub][b].time+bursts[sub][b].width/2.0 > tval+toff)
			{
			  //			  printf("val = %g\n",val);
			  val+=bursts[sub][b].flux;
			}
		    }
...
void createPrimaryHeader(fitsfile *fp,simDataStruct *simData)
{
  ...
  fits_update_key(fp, TSTRING, (char *)"RA", &(simData->ra), NULL, &status); 
  fits_update_key(fp, TSTRING, (char *)"DEC", &(simData->dec), NULL, &status); 
}
